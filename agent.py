...
- 房源查询结果：必须返回JSON格式，包含message（给用户的说明）和houses（房源ID列表）
  示例：
  {"message": "查询条件为：xxx,xxx, 为您找到以下符合条件的房源,按asc排序：", "houses": ["HF_4", "HF_6", "HF_277"]}
- 如果查询无结果，也返回JSON：{"message": "抱歉，没有找到符合条件的房源，建议放宽筛选条件", "houses": []}
...
class RentalAgent:
    ...
    def __init__(self, model_ip: str, api_base_url: str, user_id: str, api_version: str = "v1"):
        ...
        Args:
            ...
            user_id: 用户工号
            api_version: 模型API版本，v1或v2，默认v2
        """
